...
city_frequency_chart <- ggplot(ordered_city_frequency[1:15,], aes(x = reorder(primary_issuer_city, -city_count), y=city_count)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 45,color="red")) + labs(x="City", y="Frequency of City", title = "Frequency of City")
...
lapply(only_US_updated, class)

lapply(lapply(only_US_updated, unique), length)
lapply(only_US_updated, class)

model_f <- lm(log10(total_amount_sold)~factor(industry_group_type) + total_investors + log10(total_offering_amount), data = only_US_updated)

summary(model_f)

hist(log10(only_US_updated$total_amount_sold), nclass = 100)

only_US_updated$new_y <- only_US_updated$total_amount_sold/only_US_updated$total_offering_amount > 0.95

mean(only_US_updated$total_amount_sold/only_US_updated$total_offering_amount > 0.95)
mean(only_US_updated$total_amount_sold/only_US_updated$total_offering_amount > 0.90)
mean(only_US_updated$total_amount_sold/only_US_updated$total_offering_amount > 0.99)
mean(only_US_updated$total_amount_sold/only_US_updated$total_offering_amount > 0.95)
hist(only_US_updated$total_amount_sold/only_US_updated$total_offering_amount, nclass = 100)

plot(log10(only_US_updated$total_offering_amount),log10(only_US_updated$total_amount_sold))

glm(new_y ~
      factor(industry_group_type) + total_investors + log10(total_offering_amount),
      data = only_US_updated,
      family=binomial(link='logit'))


glm(new_y ~
      total_investors + log10(total_offering_amount),
      data = only_US_updated[only_US_updated$total_offering_amount >= 1000, ],
      family=binomial(link='logit'))

# new variable of buckets of total_offering_amount
only_US_updated_over_tenthousand<-only_US_updated[!(only_US_updated$total_offering_amount <= 10000),]

only_US_updated_over_tenthousand$groups <-quant_groups(only_US_updated_over_tenthousand$total_offering_amount, 4) #https://rdrr.io/rforge/dvmisc/man/quant_groups.html

levels(only_US_updated_over_tenthousand$groups) <- c("Small", "Medium","Large","Extremely Large")

only_US_updated_over_tenthousand$new_y_binary <- ifelse(only_US_updated_over_tenthousand$new_y==TRUE,1,0) 
test <- only_US_updated[!(only_US_updated$total_offering_amount <= 10000)&(only_US_updated$total_offering_amount >= 100000000) ,]

test$groups <-quant_groups(test$total_offering_amount, 4)
levels(test$groups) <- c("Small", "Medium","Large","Extremely Large")


only_US_updated_over_tenthousand$new_y_binary <- ifelse(only_US_updated_over_tenthousand$new_y==TRUE,1,0) 
glm(new_y ~
      total_investors + groups,
      data = test,
      family=binomial(link='logit'))
...
# Create the forest: Diagnostic with all variables. very bad, will force you to terminate R. 
...
#output.forest2 <- randomForest(new_y ~
      total_investors + groups,
      data = test)
